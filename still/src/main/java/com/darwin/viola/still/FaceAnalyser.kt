...
//                portrait?.face?.let { it1 -> alignFace(it1, it ) }?.let {
//                    portrait.face = it
//                }
...
//        Log.i("test","Processing face with {CropAlgorithm.$cropAlgorithm} algorithm.")

//        Log.i("test","faceSize => ${faceSize}   minFaceSize  => ${minFaceSize}")
        if (faceSize >= minFaceSize) {

            var portraitData = cropFace(face, bitmap, cropAlgorithm)
//            var portraitData = alignAndCropFace(face, bitmap, cropAlgorithm)
            var croppedBitmap = portraitData.first
//            croppedBitmap = alignFace(portraitData ,face,cropAlgorithm)
            ...
            val pixelBetweenEyes = 0.0//portraitData.second.roundDouble()
...
//            Log.i("test" ,"I am calling this function")
    ...
    private fun cropFace(
        ...
        if (cropAlgorithm == CropAlgorithm.LEAST) {
            ...
            if (cropAlgorithm == CropAlgorithm.SQUARE) {
                ...
                width = (eyeDistance / 0.35).toFloat() // previously it was 0.25
            ...
            val croppedBitmap = Bitmap.createBitmap(bitmap, finalStartX, finalStartY, finalWidth, finalHeight)
    ...
    private fun alignAndCropFace(
        ...
        val alignedFace = alignmentProcedure(bitmap, leftEyePosition, rightEyePosition)
        return cropFace(face, alignedFace, cropAlgorithm)
    }

    fun alignmentProcedure(img: Bitmap, leftEye: PointF, rightEye: PointF): Bitmap {
        ...
        if (leftEyeY > rightEyeY) {
            point3rd = PointF(rightEyeX, leftEyeY)
            direction = -1
        } else {
            point3rd = PointF(leftEyeX, rightEyeY)
            direction = 1
        }

        val a = distance(leftEye, point3rd)
        val b = distance(rightEye, point3rd)
        val c = distance(rightEye, leftEye)
        ...
        if (b != 0f && c != 0f) {
            ...
            val matrix = Matrix()
            matrix.postRotate(direction * angle)
